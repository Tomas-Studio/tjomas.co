...
<template>
  <div relative inline-flex>
    <NuxtLink v-tooltip.bottom="{ content: name, disabled: largerThanSm }" :to="to" :aria-label="`Go to ${name} page`" class="profile-link" :class="active ? 'text-brand-green' : ''">
      ...
      <span lt-sm:hidden capitalize fw500 z5 :class="active ? 'fw600' : ''" v-html="name" />
      <span :class="active ? 'scale-x-100' : 'scale-x-0'" class="absolute inset-x-1 bottom-1 h-2px bg-gradient-to-r from-brand-yellow to-brand-green dark:(via-brand-dark) origin-left duration-500" />
