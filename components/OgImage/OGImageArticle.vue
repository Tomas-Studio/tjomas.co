...
<template>
  <div class="wfull hfull px12 pt20 pb16 relative flex flex-col justify-between" style="background: #050505;">
    <div>
      <p
      ...
      <h1
        class="text-(5xl gray-300 balance) w-70% font-acorn"
      ...
      <div v-if="tags && tags?.length > 0" class="flex gap-x-4">
        <div v-for="tag of tags" :key="tag" style="font-family: 'acorn'" class="flex items-center">
          ...
          <span class="text-(gray-300 2xl) capitalize">{{ tag }}</span>
        </div>
      </div>
    </div>
    <div>
      <div class="flex justify-between">
        <div class="text-(5xl gray-300)">
          TO
        </div>
        <p v-if="date" class="text-gray-300" style="font-family: 'acorn'">
          {{ date }}
        </p>
      </div>
      <span class="text-gray-300" style="font-family: 'Urbanist';">Tunji Olakunle</span>
    </div>
    <!-- svg patterns -->
    <svg class="absolute inset-0 opacity-60" style="z-index: 0;" width="1200" height="630" viewBox="0 0 1200 630" fill="none" xmlns="http://www.w3.org/2000/svg">
